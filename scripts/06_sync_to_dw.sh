...
yearstr="${year: -2}"
repo="legis_profile_data$yearstr"
...
prepurl() {
  csv=$(basename "$1")
  house=$(echo "$csv" | sed -E "s/_.+//")
  name=$house
  desc="ACS basic indicators, CDC life expectancy estimates, PLACES Project averages, $house legislative chamber"
